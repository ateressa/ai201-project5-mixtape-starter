...
**How I tried to reproduce it** — `tests/test_search.py` already has `test_search_no_duplicates_multi_tag_song`, built specifically around a 3-tag song ("this one will duplicate in search results with the bug," per the fixture comment) with an inline note that it "should be 1, bug causes it to be 3." Running `pytest tests/test_search.py` against the unmodified code, all 5 tests passed, including that one — no reproduction. I didn't stop there, since a passing test isn't proof of no bug, only proof this test doesn't catch it. I checked the raw SQL and the live seeded DB directly: querying the `song_tags` join table for a 3-tag song's ID returns 3 rows (confirmed the join really does fan out), but `db.session.query(Song).outerjoin(song_tags, ...).all()` still returned exactly 1 `Song` object, and hitting the actual `/songs/search` endpoint through a Flask test client against the seeded DB (using the real song with the most tags) also returned `count: 1` with no duplicate titles.

**How I found the root cause** — `search_service.py:search_songs` joins `Song` to the `song_tags` association table but never filters or selects on it — tags aren't part of the search criteria at all, so the join exists but does nothing except multiply result rows once a song has more than one tag. Normally that would mean a song with 3 tags comes back 3 times. It doesn't, here, because `db.session.query(Song)...all()` uses SQLAlchemy's **legacy `Query` API**, which auto-deduplicates full-entity results by identity even when the underlying SQL produces repeated rows for the same primary key. That's a real but easy-to-miss quirk: the 2.0-style `select()`/`session.execute()` API does *not* do this automatically (it requires an explicit `.unique()` call in the same situation), so the code was only "correct" by accident of which query API it happened to use, not because it was written to guarantee uniqueness.

**The root cause** — An unused join (`song_tags`) fans out result rows per tag, and the only thing preventing visible duplicates was an implicit dedup behavior specific to the legacy `Query` object rather than an explicit guarantee in the query itself.

**My fix** — Replaced `.all()` with `.distinct()` on the query (still iterated the same way in the list comprehension at the end of `search_songs`, since `Query` objects are iterable and executing `.distinct()` doesn't require a separate `.all()` call). This makes uniqueness an explicit SQL-level `DISTINCT` clause instead of leaning on the legacy-`Query`-specific dedup behavior — the fix doesn't change what's returned today, but it makes correctness independent of which SQLAlchemy query style happens to be in use, so it survives a future refactor to `select()`/`scalars()` that would otherwise silently reintroduce the duplicates. I considered this the more robust fix even without a failing test to prove the original bug, given that the join was doing nothing but creating row multiplication in the first place. Ran the full suite afterward: all 5 `test_search.py` cases still pass, along with all of `test_streaks.py`; the two `test_playlists.py` failures present are unrelated (Issue #5, `get_playlist_songs` dropping the last song) and unaffected by this change.
